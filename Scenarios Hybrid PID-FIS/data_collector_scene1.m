...
for runNum=1:numOfRuns
    ...
    mapWidth = 50;
    mapLength = 50;
    ObstaclePositions = []; % Locations of the obstacles
    ...
    Waypoints = [InitialPosition; 10 5 -7; 20 10 -9];

    for i = 2:size(Waypoints,1)
        addMesh(Scenario,"cylinder",{[Waypoints(i,2) Waypoints(i,1) 1] [-Waypoints(i,3)-4 -Waypoints(i,3)-3.9]},[1 0 0]);
    end
    show3D(Scenario);
    hold on
    ...
    profile -memory -timer 'performance' on
    tStart=tic;
    ...
    if abort == 1
        writetable(testData,"timeOut_dataset_scene1_time"+num2str(tEnd)+".csv",'Delimiter',',','QuoteStrings',true);
    else
        writetable(testData,num2str(runNum)+"dataset_scene1_time"+num2str(tEnd)+".csv",'Delimiter',',','QuoteStrings',true);
    end
