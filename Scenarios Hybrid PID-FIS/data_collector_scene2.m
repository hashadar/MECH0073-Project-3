...
for runNum=1:numOfRuns
    ...
    mapWidth = 50;
    mapLength = 50;
    ObstaclePositions = [10 10; 24 10]; % Locations of the obstacles
    ...
    Waypoints = [InitialPosition; 12 20 -7];

    for i = 2:size(Waypoints,1)
        addMesh(Scenario,"cylinder",{[Waypoints(i,2) Waypoints(i,1) 1] [4 4.1]},[1 0 0]);
    end
    show3D(Scenario);
    hold on
    ...
    if abort == 1
        writetable(testData,"timeOut_dataset_scene2_time"+num2str(tEnd)+".csv",'Delimiter',',','QuoteStrings',true);
    else
        writetable(testData,num2str(runNum)+"dataset_scene2_time"+num2str(tEnd)+".csv",'Delimiter',',','QuoteStrings',true);
    end
